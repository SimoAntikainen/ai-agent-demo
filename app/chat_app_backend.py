...
agent = Agent('openai:gpt-4o', 
              system_prompt=(
        "Use the `web_search` tool to fetch Google-style web search results,"
        "Use the `search_the_internet` tool to fetch and read readable text from a website. The tool will return cleaned plain text. "
    ),
    deps_type=Deps,
...
deps = Deps(
            client=httpx.AsyncClient(), serper_api_key=serper_api_key
)
...
async def web_search(ctx: RunContext[Deps], query: str) -> List[SearchResult]:
    ...
    response = await ctx.deps.client.post(url="https://google.serper.dev/search", headers=headers, json=payload)
...
if __name__ == '__main__':
    ...
    uvicorn.run(
        'chat_app_backend:app', reload=True, reload_dirs=[str(THIS_DIR)]
    )
